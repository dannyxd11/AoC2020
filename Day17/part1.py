...
def get_adjacent_cubes(cube):
    x, y, z = cube
    adjacent_cubes = list(product(range(x-1, x+2, 1), range(y-1, y+2, 1), range(z-1, z+2, 1)))    
...
def get_active_cubes(cube, cubes):
    ...
    # print("adj",adjacent_cubes)
    # print("act", active_cubes)
...
with open('challenge.txt', 'r') as f:
    ...
    cubes = {(item[0],item[1],item[2]):item[3] for line in [list(zip(repeat(line[0]), line[1])) for line in enumerate(lines)] for item in [(ele[0], ele[1][0], 0, ele[1][1]) for ele in enumerate(line)]}    
